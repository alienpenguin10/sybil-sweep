...
att = json.loads(Path("attestations.json").read_text())
g = json.loads(Path("graph_data.js").read_text().split("=", 1)[1].strip().rstrip(";"))
m = g["metrics"]
...
print(f"  4 · Enforce — {len(att['attestations'])} cluster(s) ready in attestations.json")
if att["attestations"]:
...
# Prefer opening without blocking the shell
if command -v open >/dev/null 2>&1; then
  open "$ROOT/dashboard.html"
elif command -v xdg-open >/dev/null 2>&1; then
  xdg-open "$ROOT/dashboard.html"
else
  echo "Open dashboard.html in a browser: $ROOT/dashboard.html"
fi
