...
Reads ClaimEvent lists, links wallets via shared-funder and funding-burst
signals, clusters with union-find, scores risk, and emits:
  - graph_data.js     (dashboard)
  - attestations.json (on-chain publish script)
"""
...
def write_outputs(result: dict) -> None:
    ...
    js_path = OUT_DIR / "graph_data.js"
    js_path.write_text(
        "window.SYBIL_GRAPH = " + json.dumps(graph, indent=2) + ";\n",
        encoding="utf-8",
    )

    att_path = OUT_DIR / "attestations.json"
    att_path.write_text(
        json.dumps(
            {
    ...
    print(f"Wrote {js_path.name} and {att_path.name}")
